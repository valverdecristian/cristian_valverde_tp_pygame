...
class Level2():
    def __init__(self,w,h):
        ...
        self.static_background = Background(x=0,y=0,width=w,height=h,path="images/locations/all2.png")

        self.player = Player(x=100,y=400,speed_walk=10,speed_run=12,gravity=14,jump_power=30,frame_rate_ms=100,move_rate_ms=50,jump_height=140,p_scale=0.2,interval_time_jump=300)
        self.barra_salud = HealthBar(200, 10, self.player.lives, 10)
        self.datos_obtenidos = Auxiliar.leer_json(ruta="niveles.json")

        self.enemy_list = list()
        self.enemy_list.append (Enemy(x=1000,y=400,speed_walk=4,speed_run=5,gravity=14,jump_power=30,frame_rate_ms=150,move_rate_ms=50,jump_height=140,p_scale=0.24,interval_time_jump=300))
        ...
        self.enemy_list.append (Enemy(x=1100,y=100,speed_walk=3,speed_run=5,gravity=14,jump_power=30,frame_rate_ms=150,move_rate_ms=50,jump_height=140,p_scale=0.24,interval_time_jump=300))
        ...
        for coordenada in self.datos_obtenidos["segundo_nivel"]["salud"]:
    ...
    def update(self,lista_eventos):
        ...
        if not self.enemy_list and self.enemy_random < 5:
            new_enemy = Enemy.generate_random_enemy()
            self.enemy_list.append(new_enemy)
            self.enemy_random +=1
            if self.enemy_random == 5:
